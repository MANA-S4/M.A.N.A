        ...
        public IEnumerable<Contact> GetAll(int userId)
        {
            using (SqlConnection con = new SqlConnection(_connectionString))
            {
                return con.Query<Contact>(
                    @"select UserRelationId = u.UserId, u.Email, c.ContactId, c.RelationType from iti.Contacts c inner join iti.Users u on c.UserRelationId = u.UserId where c.UserId = @UserId 
                      union
                      select UserRelationId = u.UserId, u.Email, c.ContactId, c.RelationType from iti.Contacts c inner join iti.Users u on c.UserId = u.UserId where c.UserRelationId = @UserId;",
                    new { UserId = userId });
            }
        }
